...
+ Aurora changes the DB instances specified in the static or exclusion lists when DB instances change roles between writer and reader due to failover or promotion\.
...
+ Aurora doesn't change the DB instances specified in the static or exclusion lists when DB instances change roles between writer and reader due to failover or promotion\.

  For example, a custom endpoint with the type `READER` includes an Aurora Replica that is then promoted to be a writer instance\. The type of custom endpoint \(`READER`, `WRITER`, or `ANY`\) determines what kinds of operations you can perform through that endpoint\.
